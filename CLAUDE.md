...
- `actions` — `action_id` (PK), `action` (`CHECK` one of `mv/delete/rotate/resize/crop/edit/other` — **no `'missing'`**), `info` (free text; `notesToActions.sql` sets it to the note's comment verbatim — no prefix, no note ids — `' | '`-joined for several notes, NULL if none had a comment. `executeDeletions.ts` appends `' | file gone'` when the file was already missing. Older rows carry a `round1:<n>` batch label from a previous process), `request_dt` (default now), `status_dt`, `status` (`CHECK` one of `done/pending/failed`, default `pending`), `img_id` (FK → fotos).
